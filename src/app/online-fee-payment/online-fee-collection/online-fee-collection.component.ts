...
@Component({
  selector: 'app-online-fee-collection',
  templateUrl: './online-fee-collection.component.html',
  styleUrls: ['./online-fee-collection.component.scss'],
  providers: [],
})

export class OnlineFeeCollectionComponent implements OnInit {
  ...
  onSubmitPayment(type) {
    ...
    if (this.selectedFees.term.length === 0 && this.selectedFees.others.length === 0 && this.selectedFees.carryover.length === 0 && this.selectedFees.transport.length === 0 && this.selectedFees.hostel.length === 0) {
      ...
      if(type == 1){
        ...
        this.toastr.error('Select any fee to preview');
      }
      this.is_loading = 0;
      return;
    }
    else {
      this.selectedFees.type = type;
